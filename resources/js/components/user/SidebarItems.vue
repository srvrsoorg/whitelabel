            ...
            <template v-if="!link.children || !link.children.length">
              <router-link
                ...
                :class="[
                  ...
                  'text-[#31363F] group w-full my-2  flex justify-between items-center pr-6  text-tiny font-medium',
                ]"
              >
                <div class="flex items-center gap-6">
                  <p
                    class="w-[5px] h-11 rounded-tr-[20px] rounded-br-[20px]"
                  ></p>
              ...
              >
                <DisclosureButton
                  :class="[
                    isLightColor
                      ? 'hover:text-custom-700 '
                      : 'hover:text-custom-500 ',
                    'text-[#31363F] group w-full my-2 flex gap-6  items-center pr-6  text-tiny font-medium ',
                    open &&
                      (showNarrowSidebar
                        ? isLightColor
                          ? ' !text-custom-700 border-r-4 border-custom-700 !px-6'
                ...
                <DisclosurePanel>
                  ...
                  >
                    ...
                    >
                      <li
                        class="py-2 border-l list-none border-[#CBD5E0] pl-6 w-full"
                      >
                        <router-link
                          @click="toggleDropdown(link.id)"
                          :exact-active-class="[
                          ...
                          :class="[
                            'w-full   rounded-md  flex  items-center gap-x-5',
                            isLightColor
                              ? 'hover:text-custom-700'
                              : 'hover:text-custom-500',
                          ]"
                          :to="subItem.url"
                        >
                          <i class="fa-sharp fa-solid fa-circle text-[8px]"></i>
                          {{ subItem.name }}
                        </router-link>
                      </li>
                    </DisclosureButton>
        ...
        >
          <div class="flex items-center max-w-64 gap-3 w-full" v-if="user">
            ...
            <div class="max-w-40">
              ...
              <span
                class="text-sm text-gray-500 block truncate mt-1"
