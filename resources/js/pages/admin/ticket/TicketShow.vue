<template>
  ...
  <div
    ...
    class="items-center justify-between mb-3.5"
  >
    <div class="">
      <router-link
      ...
      <button class="xl:hidden" @click="toggle">
        <span
          :class="[
            ...
            'material-symbols-outlined p-2 rounded-md text-[22px]',
          ]"
        >
          account_circle
        </span>
          ...
          <template v-if="replyData.length > 0">
            <div v-for="reply in replyData" :key="reply" class="text-tiny pr-4">
              ...
              >
                ...
                <div class="w-9 h-9 min-w-9 min-h-9 rounded-full" v-else>
                  <img
                    src="/logo/gravatar.jpg"
                    alt=""
                    class="h-9 w-9 rounded-full"
                  />
                </div>
                <div class="flex sm:max-w-[80%] flex-col space-y-2">
                  ...
                  <div
                    class="bg-[#F6F6F6] text-sm text-wrap break-all rounded-lg p-3 min-w-fit"
                  >
              ...
              >
                <div class="flex justify-end items-start gap-3">
                  ...
                  >
                    <img
                      v-tooltip="reply.support_agent.email"
  ...
  >
    ...
    <template #content>
      <p class="text-tiny text-gray-600">
        Would you like to
        {{ tickets && tickets.status !== "closed" ? "close" : "open" }} the
        ticket?
      </p>
      <p
  ...
  >
    <div class="">
      <img :src="imageUrl" class="w-full h-full mx-auto" alt="" />
    </div>
  </Modal>
</template>
  
  <script>
import SmallSizeContactInfo from "@/components/admin/SmallSizeContactInfo.vue";
export default {
  name: "TicketShow",
  components: {
    SmallSizeContactInfo,
  ...
  methods: {
    ...
    async fetchTicketShow() {
      await this.$axios
        .get(`/admin/tickets/${this.$route.params.id}`)
        .then(({ data }) => {
    ...
    async fetchMessage() {
      ...
      await this.$axios
        .get(`/admin/tickets/${this.$route.params.id}/replies`)
        .then(({ data }) => {
    ...
    async sendMessage() {
      ...
      await this.$axios
        .post(`/admin/tickets/${this.$route.params.id}/reply`, formData)
        .then(({ data }) => {
    ...
    async updateTicketSatus() {
      ...
      await this.$axios
        .patch(`/admin/tickets/${this.$route.params.id}/${this.status}`)
        .then(({ data }) => {
  ...
  beforeDestroy() {
    // Clear the interval when the route is about to change
