    ...
    protected function formatMessage(string $type, string $action, array $payload): string
    {
        return match (strtolower($type)) {
            'transaction' => match (strtolower($action)) {
                ...
                'success'  => "Transaction completed successfully via {$payload['payment_gateway']}.",
                'failed'   => "Transaction failed using {$payload['payment_gateway']}.",
                'refunded' => "Transaction was refunded. Reason: " . ($payload['refund_reason'] ?? 'N/A') . ".",
                default    => "Transaction event: {$action}.",
            },
            'user' => match (strtolower($action)) {
                'created' => "New user registered with email {$payload['email']}.",
                'updated' => "User profile ({$payload['email']}) has been updated.",
                'deleted' => "User account ({$payload['email']}) has been deleted.",
                default   => "User event: {$action}.",
            },
            'server' => match (strtolower($action)) {
                'created' => "Server({$payload['name']}) has been created successfully.",
                'deleted' => "Server({$payload['name']}) has been deleted.",
                default   => "Server event: {$action}.",
            },
            'ticket' => match (strtolower($action)) {
                'created' => "A new ticket has been created: {$payload['title']}.",
                'closed' => "Ticket({$payload['title']}) has been closed.",
                'reopened' => "Ticket({$payload['title']}) has been re-opened.",
